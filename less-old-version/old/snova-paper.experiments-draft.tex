...
\begin{abstract}
\SNOVA{} is a Round~3 candidate in NIST's Additional Digital Signatures process.  Its submitted characteristic-two parameters have already been severely weakened by structure-aware wedge attacks, and the \SNOVA{} team's public repair direction introduces odd-characteristic variants whose scalar-expanded public quadratic matrices are symmetric.  We show that this symmetry creates a universal quotient inside the affine-column forgery framework of Beullens.  The existing analysis uses one homogeneous feature coordinate for every ordered power pair $(a,b)\in\{0,\ldots,\ell-1\}^2$.  Symmetry instead forces
\[
 u^\transpose (I_n\otimes S^a)P_i(I_n\otimes S^b)u
 =u^\transpose (I_n\otimes S^b)P_i(I_n\otimes S^a)u,
\]
so the homogeneous map factors through $\Sym^2_{\F_q}(A)$, where
$A=\F_q[S]\cong\F_{q^\ell}$ is viewed only as an $\ell$-dimensional
$\F_q$-space.  Its rank is therefore at most
\[
 m_1\binom{\ell+1}{2},
 \qquad m_1=\left\lceil\frac{or}{\ell}\right\rceil,
\]
rather than $m_1\ell^2$.  Over odd characteristic, the discarded directions are exactly the alternating summand $\bigwedge^2_{\F_q}(A)$.

For all nine $q=19$ parameter sets in the public Version~2.3 draft, the quotient yields residual systems ranging from 48 quadratics in 112 variables to 96 quadratics in 224 variables.  Under the same semi-regular Hashimoto/Wiedemann methodology used in the \SNOVA{} analysis, and using the stricter convention in the public analysis code, every set is estimated below its nominal NIST category, with shortfalls from 4.06 to 44.05 bits.  The same exact quotient applied to the six parameter shapes labeled Version~2.4 in the public analysis repository again places every preview set below target; the Level-III and Level-V margins are 12.43--44.05 bits.

We also give a second, Gram-matrix normal form for the official Level-I $q=19$, $\ell=r=4$ constants.  Each individual cross-column feature map is an invertible $80\times80$ matrix.  Fixing one signature column therefore reduces verifier inversion to 50 self-quadrics on a 52-dimensional affine space.  This provides an independent derivation of the attack, although the generic-MQ screen is essentially unchanged.  A restricted-Hessian audit found no common radical, no nontrivial adjoint algebra, and no low-rank bias that would presently turn this normal form into a practical forgery.

The algebraic quotient, rejection-rule bypass, and public-key reductions are exact.  The concrete bit costs remain heuristic, as do the corresponding security estimates in the \SNOVA{} materials.  This paper therefore claims a security-category or parameter break of the public odd-characteristic drafts at all three NIST security levels, not a practical full-size forgery; restoring any level requires reparameterization against the corrected rank ceiling $m_1\binom{\ell+1}{2}$.  The attack does not apply to the unsymmetrized $q=16$ public matrices.
...
\SNOVA{} is a structured Oil-and-Vinegar signature scheme designed to combine very small public keys with short signatures and fast verification.  NIST advanced \SNOVA{} to the third round of the Additional Digital Signatures process in May~2026, while explicitly allowing the surviving candidates to submit further specifications and implementations~\cite{NISTIR8610,NISTRound3}.  The scheme has not, however, had a single stable public parameter generation.

The NIST-posted Round~2 package uses $\F_{16}$ and nonsymmetrized public base matrices.  That line has already been heavily damaged: Beullens reduced the security margins by exploiting the structured whipping layer~\cite{Beullens2024SNOVA}, and the subsequent structure-aware wedge attack reports that all Round~2 parameters are broken, while noting a remaining rank conjecture in its complexity analysis~\cite{BrosEtAl2026SNOVAWedge}.  The \SNOVA{} team's later public drafts pursue an escape route in odd characteristic.  Version~2.1 added prime-field variants with symmetric public matrices; Version~2.3 supplies $q=19$ implementations and KATs; the public analysis repository later posted a six-set ``Version~2.4'' preview; and the authors' reformulation paper presents odd characteristic as a principal route for redesigning the scheme~\cite{SNOVA23,SNOVA24Preview,DingEtAl2026Reformulating}.  An odd-characteristic attack therefore tests the public repair strategy rather than the already-vulnerable submitted parameters.
...
\]
Consequently its rank is at most $K=m_1\binom{\ell+1}{2}$.  In the generic full-rank case, forging reduces directly to solving $K$ quadratic equations in
\[
 N_{\rm res}=\ell(v+o)-\left(or\ell-K\right)
\]
variables.  A defect in the affine-constraint rank cannot make the attack harder: it only leaves additional free variables, which the attacker may specialize to recover the displayed residual dimension.
\end{theorem}
...
The implementation audit supplies two independent normal forms.  First, a source-level reconstruction of the official Level-I $(28,5,19,4)$ KAT gives exact equivalence between the 80 verifier equations and 50 quadratics in 102 variables.  Second, every one of the six individual cross-column feature maps for the same public constants is an invertible $80\times80$ matrix.  Fixing one base column and solving the cross equation linearly therefore leaves 52 affine coordinates for a second column, after which only 50 self-quadrics remain, a nearly square system.  The generic underdetermined-MQ estimate is essentially unchanged.  A dedicated audit found no common radical, nontrivial adjoint algebra, or low-rank combination suggesting an immediate further shortcut.
...
\item the six Version~2.4 parameter shapes presently appear only in the public analysis repository, but the same estimates follow if their implementation retains the symmetric public-matrix rule;
\item no definitive NIST-posted Round~3 \SNOVA{} package was public when this manuscript was prepared.
\end{itemize}
The $q=16$ line lies outside this theorem, but it is not a safe fallback: the structure-aware wedge attack already reports that the submitted Round~2 parameters are broken~\cite{BrosEtAl2026SNOVAWedge}.  Our target is the public odd-characteristic redesign.

\subsection{Organization}

\Cref{sec:prelim} fixes notation and recalls the affine-column framework.  \Cref{sec:symmetry} proves the quotient and its multibase Gram extension.  \Cref{sec:attack} derives the residual MQ system, and \cref{sec:rejection} handles the verifier's rejection rule.  \Cref{sec:crossnormal} gives the cross-column 50-in-52 normal form.  \Cref{sec:complexity} evaluates both the Version~2.3 sets and the Version~2.4 preview.  \Cref{sec:validation} reports the implementation and structure audits.  \Cref{sec:related,sec:broader} discuss prior work and broader UOV-family implications, and \cref{sec:countermeasures} gives countermeasures.  \Cref{app:noAshortcut,app:multibase} give the negative extension-field audit and the multibase solver extension.
...
\begin{proposition}[Monotonicity under affine-rank defect]
\label{prop:affine-rank-monotonicity}
Fix a relation for which the homogeneous quotient rank is $\rho$, and let
$\lambda_R=\rank(W_RL_R)\le M-\rho$.  The exact elimination leaves at most
$\rho$ quadratic equations in $n\ell-\lambda_R$ variables.  By specializing
$(M-\rho)-\lambda_R$ additional free variables, the attacker obtains at most
$\rho$ quadratic equations in exactly $n\ell-M+\rho$ variables.  Consequently,
failure of the affine subsystem to attain its maximum possible rank cannot
increase the attack cost relative to the residual dimension used in our tables.
\end{proposition}

\begin{proof}
The initial elimination leaves $n\ell-\lambda_R$ free coordinates.  Because
$\lambda_R\le M-\rho$, the number $(M-\rho)-\lambda_R$ is nonnegative.
Arbitrarily fixing that many free coordinates leaves
$n\ell-\lambda_R-\bigl((M-\rho)-\lambda_R\bigr)=n\ell-M+\rho$ variables.
Specialization cannot increase the number of independent quadratic equations,
and every solution of the specialized system reconstructs to a valid solution
of the original verifier equations.
\end{proof}
...
\]
independent of $\rho$.  Thus a further quotient-rank defect decreases the equation count without changing the nominal underdetermination.  The full-rank quotient $\rho=K$ is the case observed in our experiments.
...
The proposition is an exact finite-field rank certificate for the public constants.  After absorbing the contributions of the remaining columns and offsets into the target, one may fix $x_0$ and solve the cross equation
\[
 E_{01}G_{01}(x_0,x_1)=y-Q_0(x_0)
\]
linearly in $x_1$.  For every one of 100 random choices of $x_0$ in the audit, the resulting $80\times132$ coefficient matrix had rank 80, leaving a 52-dimensional affine space.  The remaining condition is precisely
\[
 \boxed{50\text{ self quadrics in }52\text{ variables over }\F_{19}.}
\]
Direct evaluation of the official verifier agrees with this factorization.
...
For the simple public relation $\rho=e_0$, the quotient matrices attain full
...
\end{array}
\]
For the Level-I $\ell=4$ constants, all $19^3=6{,}859$ scalar projective
relations tested also produced an $80\times50$ matrix of rank 50, and 10,000
sampled module-valued relations did the same.  These certificates show that the
...
that root, 9,455 had invertible restricted Jacobian, closely matching the
...
had Jacobian rank 50, and 9,418 of 10,000 sampled coordinate strips retained
full rank.
...
degeneration that would inflate the true attack cost --- and on every rung the
official core matches the top of its control spread rather than falling below it,
so the reduced cores are neither harder nor easier than generic semi-regular
systems of the same shape.  This corroborates the operating-degree model by
...
\item the low-degree sanity-check script (Jacobian, coordinate-strip, polar, and
Hessian statistics, plus the degree-one Macaulay column-rank check) and its
logs;
\item the solving-degree audit: official $\ell=4$ cores specialized from the KAT
keys, matched random-control ensembles, and the per-degree F4 round tables with
the maximum degree reached under budget;
...
\end{itemize}
The clean official-source reimplementation will be released with the public version after coordinated disclosure; the disclosure package includes its signed result transcripts so that the team can compare them immediately against its own implementation.
...
\section{Broader Implications for Compressed UOV Families}\label{sec:broader}

The Gram viewpoint is useful beyond \SNOVA{}, but the outcome depends on the candidate's compression layer.  We record the transfer audit here and treat the full \QRUOV{} analysis separately, since its theorem, solver model, and claim boundary differ materially from the exact \SNOVA{} quotient.

\paragraph{MAYO: one exact radical, but no category break.}
For the official \MAYO{} exponent ordering, the public whipping matrices satisfy $E_{1j}=E^kE_{0j}$.  After fixing one block $x_0$ and solving the cross-star linear equations, the direction $(x_0,0,\ldots,0)$ is therefore a common polar-radical direction of the residual characteristic-two quadratic system.  In coordinates adapted to that radical, every residual equation has the form
\[
 q_s(u_2,\ldots,u_d)+c_su_1^2.
\]
Because Frobenius squaring is bijective over $\F_{16}$, one equation and one variable can be removed exactly.  Experiments on official-shape UOV base maps found radical dimension exactly one and no second radical or nontrivial adjoint algebra after elimination.  The resulting cost screens remain worse than the current direct \MAYO{} attacks, so this is a structural theorem rather than a security-category break~\cite{MAYOSpec}.

\paragraph{QR-UOV: a different, conditional geometric-resolution attack.}
The public quotient-ring representation of \QRUOV{} exposes smaller UOV systems over $\F_{127^3}$ with pulled-back parameters $(V,O,m)=(52,18,54)$, $(76,26,78)$, and $(102,35,105)$.  A public codimension-$O$ affine slice generically contains one oil point; $V$ public quadrics form a square core, two or three spare equations filter accidental roots, and the derivative left kernel recovers and publicly verifies the complete oil space.  This reduction and its extension-field toy validation are exact.  Conditional on the regularity hypotheses of the finite-field Kronecker solver of van der Hoeven and Lecerf~\cite{VanDerHoevenLecerf2021}, the leading soft bit exponents are 117.59, 170.39, and 227.59.  Those are serious asymptotic parameter-selection concerns, but not concrete fixed-parameter breaks: hidden soft-$O$ factors, fast modular composition, and enormous output representations remain unresolved.  The full analysis appears in a companion manuscript~\cite{QRUOVSpec}.
...
independent cross-column Gram normal form reduces the official Level-I verifier
to 50 quadratics on a 52-dimensional affine space.  Full quotient-rank
...
solver cost, not the symmetry quotient, rank certificates, or verifier reduction:
those parts are exact, whereas the displayed gate exponents inherit the heuristic
...
$\operatorname{Sym}^2_{\F_q}(\F_q[S])$ and the broader Gram-matrix formulation.
\fi
