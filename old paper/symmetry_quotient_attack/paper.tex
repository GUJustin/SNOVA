...
rank checks, this gives a quadratic system with the same number of equations
and variables.  The reduction preserves every verifier equation, rejection
rule, and final verification check.

The public reduction is exact, while the recovery estimates are conditional.
Under a stated root condition and an unproved regularity hypothesis inherited
from symbolic homotopy, the Version~2.3 analysis gives arithmetic exponents
$132.188$, $183.795$, and $233.844$ before an unmeasured implementation
multiplier.  The analysis uses complete systems and, where advantageous,
certified smaller systems.  Any branch-averaged figure refers to an idealized
distribution of public transcripts; public tests select one branch for a
fixed key.  These are not wall-clock or low-memory bounds.  We do not claim a
production-size solve or forgery.
\end{abstract}
...
\paragraph{Conditional recovery and branch-dependent costs.}
...
questions.  The root analysis uses either an idealized transcript model or an
exact spectrum certificate for one fixed key.  The solver analysis assumes an
unproved homotopy hypothesis $\mathsf H_{\rm hom}$.  Under these conditions,
the Version~2.3 analysis gives arithmetic exponents $132.188$, $183.795$, and
$233.844$ before the unmeasured multiplier $\kappa_{\rm hom}$
(\Cref{thm:sq-all-nine}).  Exact determinant tests choose between smaller and
complete systems.  For a fixed key this choice is deterministic.  Where both
branches contribute, the reported exponent averages them over an idealized
distribution of public transcripts.  None is a wall-clock or low-memory bound.
...
Frequency of structural and spectrum events & Distributional, or certified for a fixed key & The paper uses an idealized XOF-transcript model; a fixed key instead requires the exact public checks and a spectrum certificate.\\
Homotopy recovery and cost & Conditional & Completeness and arithmetic cost require $\mathsf H_{\rm hom}$ and the unmeasured multiplier $\kappa_{\rm hom}$.\\
