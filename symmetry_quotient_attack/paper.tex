...
This section gives the exact part of the attack as one sequence of public
operations.  The attacker first expresses every signature column in terms of
one common column.  Symmetry then identifies quadratic forms that the
unrestricted verifier treats as different.  An invertible change of output
coordinates separates the remaining quadratic equations from the affine
ones.  Once two public rank checks pass, the attacker can restrict the
remaining variables to obtain equally many quadratic equations and variables.
This section makes no assumption about finding a root of that system.
...
implementation~\cite{SNOVA23}.  The quantity $K$ is the maximum number of
distinct quadratic forms left by symmetry, as proved below.
...
belong to $\kappa_{\rm hom}$ or $\kappa_{\rm JG}$ for the chosen recovery
method.  If no tested relation passes the two rank checks, the conditional
recovery results do not apply to that key.  Once the checks pass, the steps in
this section are exact public computations and no verifier equation is
dropped.
...
\subsection{Coordinates used by the conditional solvers}
The exact reduction needs only the dimension of the symmetric square.  The
later recovery algorithms use a more structured coordinate system.  For
$A=\F_{q^d}$ and $j>0$, define
\[
