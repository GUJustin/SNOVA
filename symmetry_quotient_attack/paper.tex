...
\section{A conditional recovery theorem for every parameter set}
\label{sec:sq-overview}

The exact reduction in \Cref{cor:exact-square-restriction} constructs a square
system but does not show that the system has an acceptable root or that a
solver can find one.  The following theorem states the additional conditions
used by the paper and gives one recovery route for all nine published
parameter shapes.
...
key and chosen column relation pass the stated exact rank checks.  Suppose the
applicable bound $\bar\eta_L^U\le\eta_{128}$ holds, either through the
idealized public-key model used below or through an exact certificate for the
fixed key.  Finally, assume $\mathsf H_{\rm hom}$ for the square system that
the attack invokes.

Under these conditions, a randomized recovery algorithm solves a square
system and checks every candidate against the affine reconstruction, the
signature rejection rule, and the original verifier.  It never returns an
invalid signature.  The algorithm does not choose a Macaulay degree or assume
a unique root, a predicted corank, or a predetermined Jacobian minor.  Its
unit-leading arithmetic costs, before the unmeasured multiplier
$\kappa_{\rm hom}$, are as follows.
\begin{center}
\small
\begin{tabularx}{\textwidth}{@{}c l >{\raggedright\arraybackslash}X r r@{}}
\toprule
Level & parameters & square system & exponent & headroom\\
\midrule
...
\end{tabularx}
\end{center}
The worst costs at Levels I, III, and V are
\begin{equation}
 \boxed{142.60696,\qquad185.45088,\qquad233.84399}.   \label{eq:sq-simple-headline}
\end{equation}
These are multiplication-schedule estimates rather than complete gate counts.
The theorem does not assert that its conditions hold for every official key,
and the artifact does not run the solver at production size.
\end{theorem}

\begin{proof}
For the $\ell=4$ rows, the fresh-slice construction and
\Cref{cor:sq-l4-direct-square} give the ordinary square systems.  For the
$\ell=2$ rows, \Cref{thm:sq-l2-complete} gives the complete two-block systems.
Both cases use \Cref{thm:sq-accepted-root}, the finite-field separation bound,
the same conditional homotopy hypothesis, and complete public filtering.  The
table reports the levelwise costs and their maxima.
\end{proof}

The target-generation interpretation also uses the idealized transcript model
stated below.  None of the displayed exponents is a wall-clock estimate or a
low-memory theorem.  The B\'ezout quantities are exponential, and
$\kappa_{\rm hom}$ remains visible.

\paragraph{Root existence.}
\Cref{sec:sq-full-domain-spectrum} bounds collisions and singular roots on a
chosen square slice.  It gives an accepted nonsingular root under the stated
spectrum condition.

\paragraph{Recovery cost.}
\Cref{sec:sq-arithmetic,sec:sq-complete-square,sec:sq-l2} state the conditional
homotopy cost and instantiate it for the two public parameter families.

\paragraph{Certified optimizations.}
Later sections test smaller systems using exact public determinants.  A failed
test returns to the complete square system.  These branches improve the cost
estimates but are not needed for the theorem above.

\subsection{When does the square system contain an acceptable root?}
...
\subsection{Optional certified reductions to smaller systems}
\label{sec:sq-adaptive}

The main theorem solves every equation in the square system.  The lower cost
estimates later in the paper instead select fewer linear combinations of those
equations.  To avoid assuming that a fixed selection works, the algorithm
tests the selection using public data and returns to the complete square when
the test fails.  This subsection gives the generic statements behind that
optional branch.
...
\section{Conditional costs with certified smaller systems}
\label{sec:sq-all-nine}

The main theorem solves every equation in the square system.  The exact public
determinant tests developed above can sometimes certify a smaller system.  A
failed test returns to the complete square, so the smaller systems change the
cost estimate but not the assumptions on the public key.

\begin{theorem}[Lower conditional costs from certified smaller systems]
\label{thm:sq-all-nine}
Under the same structural, spectrum, transcript, and $\mathsf H_{\rm hom}$
conditions as \Cref{thm:sq-all-nine-simple}, an exact public determinant test
can replace the complete square by a smaller system.  If the test fails, the
algorithm solves the complete square instead.  The resulting costs are
\begin{center}
\small
\begin{tabularx}{\textwidth}{@{}c l >{\raggedright\arraybackslash}X r r@{}}
\toprule
Level & parameters & certified route & exponent & headroom\\
\midrule
I   & $(28,5,4,4)$  & smaller channel system, else full square & $128.246$ & $14.754$\\
I   & $(48,16,2,2)$ & one eigenblock, else two-block square & $132.188$ & $10.812$\\
I   & $(28,4,4,5)$  & smaller channel system, else full square & $128.246$ & $14.754$\\
III & $(40,7,4,4)$  & smaller channel system, else full square & $171.617$ & $35.383$\\
III & $(72,24,2,2)$ & complete two-block square & $183.795$ & $23.205$\\
III & $(38,5,4,5)$  & smaller channel system, else full square & $171.616$ & $35.384$\\
V   & $(50,9,4,4)$  & smaller channel system, else full square & $214.558$ & $57.442$\\
V   & $(96,32,2,2)$ & complete two-block square & $233.844$ & $38.156$\\
V   & $(52,6,4,6)$  & smaller channel system, else full square & $214.557$ & $57.443$\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{proof}
Apply \Cref{thm:sq-adaptive}.  For $\ell=2$, the Level-I test selects one
eigenblock when its determinant certificate passes and otherwise uses the
complete two-block square.  The complete square is already cheaper at Levels
III and V.  For $\ell=4$, use \Cref{lem:sq-l4-projective-jacobian} and return
to \Cref{cor:sq-l4-direct-square} when the test fails.  The artifact combines
the work of the two branches before taking logarithms and charges the union of
their spectrum events.  It conditions on the exact structural rank checks and
