...
smaller structured family.  That structure is also the main object of its
cryptanalysis.  Prior work has studied rank defects, invariant subspaces,
group actions, lifting, and multihomogeneous solving in different versions
and parameter regimes
~\cite{IkematsuAkiyama2024,LiDing2024,Beullens2024SNOVA,Cabarcas2025SNOVA,FurueEtAl2025,NakamuraEtAl2025}.
The present paper differs from these attacks in one point: it isolates what
the public symmetrization does after the common-column restriction used in
affine-column attacks.

The distinction matters because Version~2.3 changes the algebra that earlier
attacks see.  Wedge attacks exploit the characteristic-two structure of the
earlier submission~\cite{BrosEtAl2026SNOVAWedge,TsengEtAl2026Wedge}.  The
revised construction instead works over $\F_{19}$ and symmetrizes its
scalar-expanded public base matrices~\cite{SNOVA23}.  This removes the earlier
characteristic-two decomposition, but it also imposes a new equality on the
restricted quadratic features.  Existing analyses do not isolate this
equality or follow it through the complete verifier.

The question is therefore whether odd-characteristic symmetrization removes
the structural weakness exposed by affine-column attacks.  Our answer is
precise.

\begin{quote}
\centering
\textbf{Under the public common-column restriction, the Version~2.3
symmetrization forces the restricted verifier through a symmetric-square
quotient.  An exact public output split then converts the verifier equations
into square residual systems for every published parameter shape that passes
the stated public rank preflights.}
\end{quote}
...
The reduction starts with a public affine relation that writes every matrix
column in terms of one common column.  Substitution turns each matrix-valued
quadratic feature into a scalar quadratic polynomial.  Symmetry then identifies
the labels $(\xi,\eta)$ and $(\eta,\xi)$ before the outer public map is
applied.  A basis computation exposes the resulting quadratic image and an
invertible change of output coordinates separates that image from the affine
and target-consistency coordinates.  No verifier equation is discarded.

Once a target passes the consistency coordinates, the remaining equations
are restricted to a public slice with as many variables as quotient
quadratics.  Root-spectrum bounds control whether such a slice contains an
accepted nonsingular root.  The recovery theorem then applies a
finite-cardinality symbolic-homotopy algorithm under
$\mathsf H_{\rm hom}$.  A failed nonlinear solve triggers a restart; any
candidate that is recovered is reconstructed and checked by the original
verifier.  This separation keeps the algebraic reduction exact even where the
solver analysis remains conditional.
