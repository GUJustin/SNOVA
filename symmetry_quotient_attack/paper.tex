...
\section{Instantiating the conditional recovery theorem}
\label{sec:sq-arithmetic}

This section gives the arithmetic model and then instantiates the complete
square-system route for the $\ell=4$ and $\ell=2$ parameter families.
...
$\kappa_{\rm hom}$.
A narrower normalization that counts only variable products appears in the
artifact but supports no theorem in this paper.
...
\subsection{Complete square systems for \texorpdfstring{$\ell=4$}{ell=4}}
...
\subsection{Zero-offset systems for \texorpdfstring{$\ell=2$}{ell=2}}
\label{sec:sq-l2}
...
\subsubsection{The complete two-block system}
...
\subsubsection{The smaller Level-I system and its exact fallback}
...
\section{Certified smaller systems and optimized costs}
\label{sec:sq-l4}

The $\ell=2$ family admits the smaller Level-I system above.  We now give the
corresponding certified reduction for $\ell=4$, then combine both families in
one all-nine cost table.
...
If the determinant test fails, the algorithm uses the complete ordinary
square of \Cref{cor:sq-l4-direct-square}.  \Cref{thm:sq-adaptive} combines the
two branches.  The resulting costs appear in the all-nine table below.  We
report only profiles whose complete numerical inputs are regenerated by the
artifact.  No sixteen-orbit or output-size-optimized numerical branch is
claimed.

\subsection{Optimized costs for all nine parameter shapes}
