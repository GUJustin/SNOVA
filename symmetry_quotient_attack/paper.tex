...
\paragraph{An exact reduction of the public verifier.}
...
linear combinations cannot restore the missing quadratic forms.  We then use
public row reduction to separate the surviving quadratics from the linear
conditions and target checks.  For every published parameter shape that
passes the stated public rank checks, the remaining nonlinear problem has
equally many equations and variables.  The attack retains reconstruction, the
signature rejection rule, and the original verifier
(\Cref{thm:symmetry-quotient,prop:exact-affine,thm:structural-preflight}).

\paragraph{Conditional cost estimates.}
Producing a square system does not prove that it contains an acceptable root
or that a solver can find one efficiently.  We treat these as separate
questions.  Under the stated root condition and the unproved homotopy
hypothesis $\mathsf H_{\rm hom}$, a randomized algorithm covers all nine
parameter shapes and never returns an invalid signature.  Exact public
determinant tests can select smaller systems, with the complete square system
as the fallback.  The resulting worst arithmetic exponents are $132.188$,
$183.795$, and $233.844$ before the unmeasured multiplier
$\kappa_{\rm hom}$ (\Cref{thm:sq-all-nine}).  The root condition comes from
either an idealized transcript model or an exact certificate for one fixed
key.  A separate Level-I analysis of Just Guess retains its own unmeasured
multiplier $\kappa_{\rm JG}$ (\Cref{prop:jg-level1-gates}).  These are
conditional cost estimates, not production forgeries.

\paragraph{Evidence and design implications.}
The artifact checks the algebra and cost calculations, instantiates the public
reduction on one official test key, and exercises the full attack
...
Section~2 first defines the full signing and verification maps.  The attack
then starts from the public column relation used by earlier forgery attacks:
inside every matrix-valued signature block, all columns are written as public
affine functions of one stacked column.  This is a restriction chosen by the
attacker.  It is not an identity of the unrestricted verifier.

After this restriction, every ordered pair of public matrix multipliers gives
a scalar quadratic polynomial.  Symmetry of the public base matrices makes
the polynomials for the two orders equal.  One group of sixteen ordered pairs
therefore contributes at most ten distinct quadratics when the block height is
four; one group of four contributes at most three when the block height is
two.  This identification occurs before the public map combines the forms, so
later linear combinations cannot restore the distinction
(\Cref{thm:symmetry-quotient}).

The attacker next computes an invertible public change of verifier outputs.
Some new coordinates contain the surviving quadratic forms.  The others are
linear conditions on the signature, together with any conditions required of
the target.  No verifier equation is dropped.  Solving the linear conditions
gives a public affine space, and restricting that space to as many variables
as surviving quadratics gives a square system.  Two public rank tests certify
this exact reduction for a fixed key (\Cref{prop:exact-affine,cor:exact-square-restriction}).

The exact argument ends there.  It does not prove that the square system has
an acceptable root or that such a root can be found within the claimed work.
The later recovery analysis states separate hypotheses for root existence and
for the solver.  Exact determinant tests may select smaller systems, while the
complete square system remains the fallback.

Every recovered root is converted back into a candidate signature, checked
against the public format rule, and passed to the original verifier.  A failed
rank test, target check, solve, or final verification causes a restart.  The
public restriction, output change, reconstruction, and verifier check are
exact once the rank tests pass.  The root and cost estimates remain
conditional, and the artifact contains no signature at a published parameter
size.
...
solved.  The displayed work is the arithmetic ledger multiplied by the
unmeasured factor $\kappa_{\rm hom}$.
...
The table gives the reported arithmetic exponents before the unmeasured
multiplier $\kappa_{\rm hom}$.
\begin{center}
\small
...
\end{equation}
These are multiplication-schedule estimates, not wall-clock or low-memory
bounds.  The theorem does not assert that its conditions hold for every
official key, and the artifact does not run a production-size nonlinear
solver.
\end{theorem}
...
factorization, communication, and peak state remain outside it.
\Cref{app:conditional-solver} records the solver coordinates and homotopy
adaptation.  \Cref{sec:jg-finite-gate} leaves $\kappa_{\rm JG}$ unmeasured, while
...
All nine published shapes meet the dimension requirements of the exact square
restriction (\Cref{cor:exact-square-restriction}).  Applying the recovery
algorithms to a key also requires the stated public preflights
(\Cref{thm:sq-all-nine}).  The structural reduction is exact;
...
the tower basis and the displayed multiplication recurrences.  The unmeasured
factor $\kappa_{\rm hom}$ contains basis conversion, bulk evaluation and
...
ledger from \Cref{sec:sq-preparing} and precede the unmeasured multiplier
$\kappa_{\rm hom}$.
\end{corollary}
...
The exact ledger gives the following sufficient conditions for the unmeasured
homotopy multiplier:
\[
...
\]
These inequalities are rounded downward.  The analysis does not assert a
particular value for any of these multipliers.
