...
\hypersetup{
  ...
  citecolor=deepblue,
  urlcolor=deepblue,
  pdftitle={Symmetric Public Matrices Create Smaller Forgery Systems in Odd-Characteristic SNOVA},
...
\newcommand{\attacktitle}{Symmetric Public Matrices Create Smaller Forgery
  Systems in Odd-Characteristic \texorpdfstring{\SNOVA}{SNOVA}}
...
In this paper, we analyze the common-column relation already used in
affine-column forgery attacks.  If the public matrix algebra has dimension
$d$, symmetry reduces the number of distinct restricted quadratic forms from
$d^2$ to $d(d+1)/2$.  Thus sixteen forms become ten for the official
$\ell=4$ parameters, and four become three for $\ell=2$.  We then give an
invertible public change of output coordinates that separates these quadratics
from the remaining affine constraints and target checks.  For every published
$q=19$ parameter shape that passes the stated public rank checks, the attack
reduces forgery to a quadratic system with the same number of equations and
variables.  It preserves every verifier equation, rejection rule, and final
verification check.

The algebraic reduction is exact.  The estimates for solving the resulting
systems are conditional.  Under an explicit unproved hypothesis inherited
from symbolic homotopy, we obtain arithmetic exponents $132.188$, $183.795$,
and $233.844$ before unmeasured implementation multipliers.  A separate
Level-I analysis based on Just Guess gives a cost below
$2^{135.425325}\kappa_{\rm JG}$ when its stated root-count condition holds.
These are conditional estimates, not production-size forgeries.  The artifact
checks the public reduction on one official test key and completes the attack
only for toy parameters.
\end{abstract}
...
al. give a UOV key-recovery attack in arbitrary characteristic using a reduced
symmetric algebra~\cite{JinEtAl2026}.  Ran's latest analysis gives a different
odd-characteristic generalization using symmetric forms, but explains that
SNOVA's added structure requires a separate treatment~\cite{Ran2026}.  Ding,
Li, and Tao formulate a multihomogeneous wedge attack on SNOVA that is valid in
any characteristic, although their concrete SNOVA estimates cover only the
even-characteristic Round~2 parameters~\cite{DingLiTao2026}.  None of these
works analyzes the symmetric public matrices in the current $q=19$
construction together with the affine-column forgery relation.

The question is therefore what happens when an affine-column attacker imposes
the same common-column relation on the new symmetric verifier.  We prove that
symmetry makes pairs of restricted quadratic forms identical.  An invertible
public change of output coordinates then separates these quadratics from the
remaining affine constraints and target checks.  For every published $q=19$
parameter shape that passes the stated public rank checks, this leaves a
quadratic system with the same number of equations and variables.
...
\paragraph{How symmetry reduces the restricted verifier.}
...
\paragraph{Reducing forgery to equally many equations and variables.}
...
quadratic system with equally many equations and variables on a public slice.
Reconstruction, rejection, and the final unmodified verifier check remain part
of the attack
(\Cref{prop:exact-affine,thm:structural-preflight,thm:sq-all-nine-simple}).

\paragraph{Conditional cost estimates.}
We give a randomized recovery algorithm that never returns an incorrect
candidate and does not require choosing a solving degree.  It covers every
published parameter shape.  A second version uses exact public determinant
tests to select smaller systems.  The corresponding worst levelwise arithmetic
...
$\kappa_{\rm JG}$ (\Cref{prop:jg-level1-gates}).  These are conditional
break-even calculations, not complete classical-gate breaks.

\paragraph{Evidence and design implications.}
The artifact checks the algebra and cost calculations, instantiates the public
reduction on one official known-answer-test key, and exercises the full attack
only at an unofficial toy shape.  It contains no production-size solve or
forgery (\Cref{tab:evidence-hierarchy}).  We also derive the minimum output
increases required to remove the dimension guarantee while retaining the
current common-column architecture.  The general condition requires increases
between $45.00\%$ and $60.71\%$.  The stronger condition for a column relation
with zero constants requires increases between $101.5625\%$ and $144.00\%$
(\Cref{thm:upgrade-combined-floor,thm:upgrade-zero-offset-floor}).  These are
necessary dimension conditions for the stated repair goals, not sufficient
conditions for security.

The distinction between exact and conditional claims is intentional.  The
quotient, output change,
reconstruction, and verifier check are exact public computations once their
displayed rank checks pass.  The required matrix layouts and ranks are
deterministic public checks, but this paper does not estimate how often
official keys pass them.  Claims about the number of acceptable roots use
either an idealized transcript model or a certificate for one fixed key.
Recovery costs additionally depend on $\mathsf H_{\rm hom}$ and the unmeasured
implementation multipliers.  Nothing in the paper establishes an
unconditional production-size forgery.
...
all columns as public affine functions of one stacked column $u$.  After this
substitution, the verifier has $N_0=n\ell$ scalar variables.  Each ordered pair
of public matrix powers now defines a scalar quadratic polynomial
$q_{i,\xi,\eta}(u)$.  The attacker chooses this restriction.  It is not an
identity for the unrestricted matrix-valued map.

The new observation occurs before the public map combines these polynomials.
Because the Version~2.3 base matrices are symmetric, reversing the two matrix
powers does not change the restricted polynomial.  In symbols,
$q_{i,\xi,\eta}=q_{i,\eta,\xi}$.  This is the symmetric-square quotient.  For
$\ell=4$, each group of sixteen ordered pairs contributes at most ten distinct
quadratics.  For $\ell=2$, four pairs contribute at most three.  Later public
linear combinations cannot recover the distinctions that symmetry removed.

For a fixed public column relation $R$, the attack computes how the remaining
quadratics contribute to the verifier outputs.  Standard linear algebra gives
an invertible public change of output coordinates.  Some new coordinates
contain the quadratic forms.  The others contain the remaining affine
constraints and checks that the target is reachable.  No verifier equation is
discarded or replaced.

The affine constraints define a public space of possible solutions.  The
attack chooses a subspace $L$ with one variable for each remaining quadratic
equation.  The nonlinear part therefore has equally many equations and
variables.  Public rank tests check that the output change and this choice of
$L$ are valid.  Separate target and rejection checks ensure that a recovered
point corresponds to the official target distribution and satisfies the
signature-format rules.

What remains is to solve the quadratic system, and this is where the
conditional analysis begins.  The paper states conditions under which the
chosen subspace contains an acceptable solution with a full-rank Jacobian.
Under $\mathsf H_{\rm hom}$, a symbolic-homotopy algorithm recovers such
solutions with the reported arithmetic costs.  Exact determinant tests may
identify smaller systems, but the complete system remains the fallback.  The
solver assumptions prevent these estimates from being unconditional breaks.

Finally, the attack reconstructs a signature candidate from each recovered
solution, applies the public rejection rules, and runs the unmodified verifier.
A failed target check, rank check, solve, or final verification causes a
restart.  The restriction, output change, reconstruction, and verifier check
are exact public computations.  The number of acceptable solutions and the
cost of finding them at full size retain the conditions stated in the
theorems.
