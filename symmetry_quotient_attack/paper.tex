...
target-consistency constraints.  The result is a square quadratic system that
preserves every verifier equation, the rejection rule, and final verification.
This statement concerns the restricted verifier, not the unrestricted
matrix-valued map.

All nine published shapes meet the dimension requirements
(\Cref{thm:sq-all-nine-simple}), but applying the recovery algorithms to a key
also requires the stated public preflights.  The structural reduction is exact;
the homotopy and Just Guess estimates require their stated hypotheses and
unmeasured multipliers.  The artifact contains no production-size solve or
forgery (\Cref{tab:evidence-hierarchy}).
